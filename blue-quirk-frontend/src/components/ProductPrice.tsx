...
const SIZES = {
  ...
  xl: { current: "text-[3rem] leading-none md:text-[3.75rem]", previous: "text-base md:text-lg", badge: "text-xs px-2 py-0.5" },
} as const;
...
export default function ProductPrice({
  ...
  smallUnit?: boolean;
  // Stack the previous (crossed-out) price + discount chip on a line BELOW the
  // current price, instead of all three inline on one baseline.
  stackMeta?: boolean;
  className?: string;
}) {
  ...
  // Previous price + discount chip on a line above; current price beneath it.
  if (stackMeta) {
    return (
  ...
  // Default: previous price, current price and discount chip inline on one baseline.
  return (
